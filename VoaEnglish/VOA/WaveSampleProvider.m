...
//    clientFormat.mReserved           = 0;
    ...
    SInt64 NUM_FRAMES_PER_READ =binSize;
    float *audio[extAFNumChannels];
	
    for (int i=0; i < extAFNumChannels; i++) {
        NSLog(@"%f",(float)NUM_FRAMES_PER_READ);
        audio[i] = (float *)malloc(sizeof(float)*(float)NUM_FRAMES_PER_READ);
    }
	int packetReads = 0;
...
- (float *)dataForResolution:(int )pixelWide length:(int *)length
{
...
//	int secPerPixel = (int)lengthInSec / (int)pixelWide;
    ...
    int rangeLength=200;
    pixelWide=normalizedData.count/rangeLength;
    NSLog(@"rangelength:%d",rangeLength);
	int retLength = pixelWide;
	float *ret = (float *)calloc(sizeof(float),retLength);
	int k = 0;
	//for(int r = 0; r < normalizedData.count; r += rangeLength) {
    for(int r = 0; r < rangeLength*pixelWide; r += rangeLength) {
		float valMax = 0;
	...
	*length = k;
    NSLog(@"%d,%d,%d",normalizedData.count,rangeLength,k);
	return ret;
