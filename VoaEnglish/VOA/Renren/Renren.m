...
- (void)widgetDialog:(RODialog *)dialog withOperateType:(RODialogOperateType )operateType{
    ROResponse *response = nil;
     ...
     if ([dialog isKindOfClass:[ROWebNavigationViewController class]]) {
...
//    NSLog(@"aaa");
    [self logout:self.renrenDelegate];
//    NSLog(@"bbb");
    if (ROWebviewController) {
        
        MBProgressHUD * HUD = [[MBProgressHUD alloc] initWithView:ROWebviewController.view];
        HUD.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"37x-Checkmark.png"]];
        HUD.mode = MBProgressHUDModeCustomView;
        HUD.labelText = @"登出成功";
        [ROWebviewController.view addSubview:HUD];
        [HUD show:YES];
//        NSLog(@"bbb");
        [ROWebviewController performSelector:@selector(close) withObject:nil afterDelay:1];
    }
}
...
{
//    NSLog(@"param3:%@",[params valueForKey:@"action_name"]);
    NSString *dialogURL = [kDialogBaseURL stringByAppendingString:action];
	[params setObject:_appId forKey:@"app_id"];
	[params setObject:@"touch" forKey:@"display"];
	[params setObject:titleId forKey:@"titleId"];
//    [params setObject:@"104" forKey:@"myappId"];
...
//        NSLog(@"111");
    ...
    ROWebviewController = [[ROWebNavigationViewController alloc] init];
    ROWebviewController.serverURL = dialogURL;
    ROWebviewController.params = params;
    ROWebviewController.delegate = self;
//    NSLog(@"222");
    [ROWebviewController show];
}
